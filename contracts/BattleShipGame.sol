...
contract BattleShipGame {
    ...
    function shot(uint256 _gameId, uint256 _row, uint256 _col) public {
        ...
        // if ther was accusationTirgger active -> now there is a move (shot)  
        if(gameList[_gameId].accuser != address(0)){
            emit ResolveAccuse(_gameId, gameList[_gameId].accuser);
            gameList[_gameId].accuser = address(0);  // set accuser = 0
    ...
    //
    function shotResult( // function to comunicate the result of the shot
        uint256 _gameId,
        uint256 _row,
        uint256 _col,
        uint256 _result,
    ...
    // select the correct node (siblings of the node to verify) to reconstruct MerkleTree back
        uint256 index = _row * gameList[_gameId].boardSize + _col;
        for (uint i = 0; i < _merkleProof.length; i++) {
            if (index % 2 == 0) { // if index is odd -> markleProof[i] is on the right from Node
                hashValue = keccak256(
                    abi.encodePacked(hashValue, _merkleProof[i])
                );
            } else {
                hashValue = keccak256( // if index is even -> markleProof[i] is on the left from Node
