...
App = {
  ...
  bindEvents: async function () { //function with all the function linked with the button of HTML 
    // button to start the game creation menu:
    ...
    if (!boardSize || !shipNumber || !ethAmmount) { // check on the input value
      ...
      App.contracts.BattleShipGame.deployed().then(async function (instance) {
        ...
        if (gameId < 0) {
          ...
          App.handleEvents();
          console.log("gameCreation function calls SC.createGame");
        }
      }).catch(function (err) {
  ...
  joinGame: function (isRandom) { // function to handle a join game
    ...
    if (isRandom == true) { // check on the gameId
      selectedGameId = 0;
      $('#setUpNewGame').hide();
      console.log("joinGame function (random) call SC.joinGame");
    }
    else{
      selectedGameId = $('#selectedGameId').val();
      console.log("joinGame function (gameID) calls SC.joinGame");
    }
  ...
  showAcceptEthAmount: function () { // function to show the information of the board to accept the game
    ...
    $('#acceptAmount').show();
    console.log("showAcceptEthAmount function shows menu");
  },
  ...
  acceptEthAmount: function () { // function to accept the ethereum amount
    App.contracts.BattleShipGame.deployed().then(async function (instance) {
      newInstance = instance
      return newInstance.amountEthDecision(gameId, true, { value: (ethAmmount*1000000000000000000) }); // true = bool _response
    }).then(async function (logArray) {
      App.handleEvents();
      console.log("acceptEthAmount func calls SC.amountEthDecision with _response = true");
    }).catch(function (err) {
      console.log(err.message);
    });
  },
  ...
  refuseEthAmount: function () { // function to refuse the ethereum amount
    App.contracts.BattleShipGame.deployed().then(async function (instance) {
      newInstance = instance
      return newInstance.amountEthDecision(gameId, false); // false = bool _response
    }).then(async function (logArray) {
      App.backToMainMenu();
      console.log("acceptEthAmount func calls SC.amountEthDecision with _response = false");
    }).catch(function (err) {
      console.log(err.message);
    });
  },
    ...
    await newInstance.allEvents(
      (err, events) => {
        ...
        if (events.event == "AmountEthResponse" && events.args._gameId.toNumber() == gameId && events.blockNumber != lastBlock) {
          ...
          console.log("AmountEthResponce event");
          ...
          console.log("ShotShip event shows Opponent's attack on my board, and calls SC.shotResult. Create MerkleProof and getHush of the attacked cell");
        }
          ...
          console.log("ShotResult event shows result of attack to Attacker, after approving that MerkleRoot == HashedValue");
        }
          ...
          console.log("Game ended .winner.transfere(ETH)");
        }
          ...
          iWasAccused = true;
          console.log("AccusationTrigger event _accused");
        }
          ...
          // you accuse and your accuse was notified
          lastBlock = events.blockNumber;
          accusationBlock = events.blockNumber + 5;
          console.log("AccusationTrigger event _accuser");
        }
        ...
        if(accusationBlock != null){ // if you have an active accuse
          if(accusationBlock >= events.blockNumber){ // if the current block is lower than the accusationBlock 
            ...
            App.contracts.BattleShipGame.deployed().then(async function (instance) {
              ...
              console.log("ResolveAccuse event calls SC.verifyAccuse. If accusationTime <= block.number ==> accuser.transfer(ETH)")
            }).catch(function (err) {
              console.log(err.message);
            });
          }
  ...
  createBoardTable: function () { // function to create a board for the placement phase
    ...
    console.log("createBoard function");
  },
  ...
  placeShip: function (event) { // function to add a ship in a specific position of the board
    ...
    console.log("PlaceShip function");
  },
  ...
  submitBoard: function () { // function tu submit the board to the opponent
    ...
    App.contracts.BattleShipGame.deployed().then(async function (instance) {
      ...
      gameStarted = true;
      App.startBattlePhase();
      console.log("submitBoard function calls SC.submitBoard");
    }).catch(function (err) {
      console.log(err.message);
    });
  ...
  accuseOpponent: function () { // function to accuse the opponent
    if(isMyTurn){
      alert("You can't accuse the opponent in your turn!");
      return;
    }
    ...
    App.contracts.BattleShipGame.deployed().then(async function (instance) {
      ...
      console.log("accuseOpponent function calls SC.accuseOpponent");
    }).catch(function (err) {
      console.log(err.message);
    });
  },

  startBattlePhase: function () { // function to show the opponentBoard and start the shot phase
    ...
    console.log("startBattlePhase function show the opponent's board on page");
  },
  ...
  handleShot: function (event) { // function to handle the shot on a cell
    ...
    if (opponentBoardMatrix[cellRow][cellCol] == 1) {
      alert("You have already attacked this cell, try again!");
      return;
    }
    ...
    App.contracts.BattleShipGame.deployed().then(async function (instance) {
      ...
      console.log("handleShot function calls SC.shot");
    }).catch(function (err) {
      console.log(err.message);
  ...
  createMerkleTree: function () {
    ...
    for (let i = 0; i < boardSize; i++) {
      for (let j = 0; j < boardSize; j++) {
        var r = Math.floor(Math.random() + Number.MAX_SAFE_INTEGER) // Hiding Hash Function
        let toHash = myBoardMatrix[i][j].toString() + r;
        temp.push(window.web3Utils.soliditySha3(toHash));
      }
    }
    merkleTreeMatrix.push(temp);

  // creation of the inside node (included the root) from the previous level
    while (temp.length > 1) {
      const nextLevel = [];
      for (let j = 0; j < temp.length; j += 2) { 
        const leftChild = temp[j];
        const rightChild = temp[j + 1];
        const toHash = leftChild + rightChild.slice(2) // Remove the "0x" prefix from the bytes32 values
        nextLevel.push(window.web3Utils.soliditySha3(toHash));
      }
      temp = nextLevel;
      merkleTreeMatrix.push(nextLevel);
    }

    console.log("createMerkleTree function by submitBoard");
  ...
  getHash: function (row, col) {
    ...
    console.log("getHash function by ShotShip event");
  ...
  createMerkleProof: function (row, col) {
    ...
    console.log("createMerkleProof function by submitBoard");
    return merkleProof;
  }
};
